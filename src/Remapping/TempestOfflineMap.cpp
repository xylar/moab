...
void TempestOfflineMap::Initialize() {
...
///////////////////////////////////////////////////////////////////////////////
moab::ErrorCode TempestOfflineMap::GenerateOfflineMap( Mesh& meshInput, Mesh& meshOutput, Mesh& meshOverlap,
...
*/
    ...
    // if (created_local)
    {
	    ...
	    this->InitializeSourceDimensionsFromMesh(meshInput);
    ...
    double dTotalAreaInput_loc = meshInput.CalculateFaceAreas();
    ...
    if (eInputType == DiscretizationType_FV) {
        this->SetSourceAreas(meshInput.vecFaceArea);
    }

    // Calculate Face areas
    if (!pcomm->rank()) dbgprint.printf(0, "Calculating output mesh Face areas\n");
    Real dTotalAreaOutput_loc = meshOutput.CalculateFaceAreas();
    ...
    if (eOutputType == DiscretizationType_FV) {
        this->SetTargetAreas(meshOutput.vecFaceArea);
    }
    ...
    for (unsigned i = 0; i < meshOverlap.vecSourceFaceIx.size(); i++) {
    ...
    if (meshInput.faces.size() - ixSourceFaceMax == 0 //&&
    ...
    // Check for reverse correspondence in overlap mesh
        ...
        meshOutput.faces.size() - ixSourceFaceMax == 0 //&&
    ...
    Real dTotalAreaOverlap_loc = meshOverlap.CalculateFaceAreas();
...
*/
    ...
    if ((eInputType  == DiscretizationType_FV) &&
        ...
        this->InitializeSourceCoordinatesFromMeshFV(meshInput);
        this->InitializeTargetCoordinatesFromMeshFV(meshOutput);

        // Construct OfflineMap
        if (!pcomm->rank()) dbgprint.printf(0, "Calculating offline map\n");
        LinearRemapFVtoFV(meshInput, meshOutput, meshOverlap, nPin, *this);
...
//#pragma warning "NOTE: VERIFICATION DISABLED"
    ...
    if (strInputData != "") {
        dbgprint.printf(0, "Applying offline map to data\n");
