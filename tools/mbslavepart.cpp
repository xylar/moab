...
int main(int argc, char* argv[])
{
  ...
  double tolerance=1e-10, btolerance=1e-6;
  std::string masterfile, slavefile, outfile;
  ...
  opts.addOpt<void>("overwrite,O", "Overwrite existing partitions in the slave mesh", &overwrite);
  ...
  // TODO: Right now, specific to Climate problems - generalize later
  Tag parttag;
  ...
  std::map<EntityHandle, int> mpartvals;
  error = mbCore->get_entities_by_type_and_tag(masterfileset, MBENTITYSET, &parttag, NULL, 1, msets, moab::Interface::UNION);
  if (msets.size() == 0) {
  ...
  for (unsigned i=0; i < msets.size(); ++i) {
    ...
    // std::cout << i << ": Found " << msetelems.size() << " elements with partID = " << partID << std::endl;
  }
  ...
  int npoints_notfound=0;
  {
    ...
    for (size_t ie=0; ie < selems.size(); ie++) {
      ...
      if (!leaf_elems.size()) std::cout << ie << ": " << " Number of leaf elements = " << leaf_elems.size() << std::endl;
      ...
      for (size_t il=0; il < leaf_elems.size(); ++il) {
        ...
        // std::cout << "\t" << ie << ": " << il << ": Minimum = " << dist << " << And Distance = " << locdist << std::endl;
      ...
      if (pinelem < 0) {
        std::cout << ie << ": Point is not contained in the mesh." << std::endl;
        npoints_notfound++;
      }
      else {
        spartvals[ mpartvals[ leaf_elems[pinelem] ] ].insert(selems[ie]);
        // std::cout << ie << ": Element with partition set " << mpartvals[leaf_elems[pinelem]] << " added to set." << std::endl;
      }
    }
  ...
  for (std::map<int, moab::Range >::iterator it = spartvals.begin(); it != spartvals.end(); ++it) {
    ...
    // std::cout << "Adding " << it->second.size() << " elements to part = " << it->first << std::endl;
    error = mbCore->tag_set_data(parttag, &pset, 1, &partID);MB_CHK_ERR(error);
  }

  error = mbCore->write_file("mpas_slave.vtk");MB_CHK_ERR(error);
