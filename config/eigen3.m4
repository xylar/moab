...
[
  AC_ARG_ENABLE(eigen3,
                AC_HELP_STRING([--enable-eigen],
                               [build with Eigen3 linear algebra support]),
    [case "${enableval}" in
      yes)  enableeigen=yes ;;
       no)  enableeigen=no ;;
  ...
  if (test $enableeigen = yes); then
    ...
    if (test $enableeigen = yes); then
       ...
       if (test $enableeigen = yes); then
         EIGEN_INCLUDES="-I$EIGEN_INC"
         CPPFLAGS="$EIGEN_INCLUDES $CPPFLAGS"
         DISTCHECK_CONFIGURE_FLAGS="$DISTCHECK_CONFIGURE_FLAGS --with-eigen=\"${EIGEN_INC}\""
