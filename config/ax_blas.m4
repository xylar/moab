...
if test $ax_blas_ok = no; then
  AC_CHECK_LIB(blas, [$sgemm], [ax_blas_ok=yes;LDFLAGS="-framework vecLib $LDFLAGS"], [ax_blas_ok=no],
   [-framework vecLib])
fi
if test $ax_blas_ok = no; then
  AC_CHECK_LIB(blas, [$dgemm], [ax_blas_ok=yes;LDFLAGS="-framework Accelerate $LDFLAGS"], [ax_blas_ok=no],
