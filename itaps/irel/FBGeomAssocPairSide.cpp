  ...
  id(p_id)
{
  ...
  FBiGeom_getTagHandle(instance_, GLOBAL_ID_TAG_NAME, &gid_tag, &result,
                       strlen(GLOBAL_ID_TAG_NAME));
  if (result == iBase_TAG_NOT_FOUND) {
    FBiGeom_createTag(instance_, GLOBAL_ID_TAG_NAME, 1, iBase_INTEGER,
...
int FBGeomAssocPairSide::create_relation_side()
{
  ...
  FBiGeom_getTagHandle(instance_, rel_tag_name.c_str(), &relation_tag, &result,
                       rel_tag_name.size());
  if (result == iBase_TAG_NOT_FOUND) {
    FBiGeom_createTag(instance_, rel_tag_name.c_str(), 1, iBase_ENTITY_HANDLE,
                                          ...
                                          int *entities_size)
{
  ...
  FBiGeom_getEntities(instance_, 0, this_type, entities, entities_alloc,
                                      ...
                                      int *entities_size)
{
  ...
  FBiGeom_getEntities(instance_, set_handle, this_type, entities,
                                     ...
                                     int *ent_types_size)
{
  ...
  FBiGeom_getArrType(instance_, entities, entities_size, ent_types,
                                           ...
                                           int num_entities, void *values)
{
  ...
  FBiGeom_getArrData(instance_, entities, num_entities, relation_tag,
                                           ...
                                           int num_sets, void *values)
{
  ...
  for (int i = 0; i < num_sets; i++) {
    FBiGeom_getEntSetData(instance_, sets[i], relation_tag,
                                           ...
                                           int num_entities, const void *values)
{
  ...
  FBiGeom_setArrData(instance_, entities, num_entities, relation_tag,
                                           ...
                                           int num_sets, const void *values)
{
  ...
  for (int i = 0; i < num_sets; i++) {
    FBiGeom_setEntSetData(instance_, sets[i], relation_tag, data, size,
                          &result);
    data += size;
    PROCESS_ERROR;
  }
                                  ...
                                  int num_entities, int *values)
{
  ...
  FBiGeom_getArrData(instance_, entities, num_entities, gid_tag,
...
int FBGeomAssocPairSide::get_gids(iBase_EntitySetHandle *sets, int num_sets,
                                  int *values)
{
  ...
  for (int i = 0; i < num_sets; i++) {
    FBiGeom_getEntSetData(instance_, sets[i], gid_tag,
                          reinterpret_cast<void**>(&data),
                          &values_alloc, &values_size, &result);
    data += values_size;
    PROCESS_ERROR;
